...
export default {
  ...
  mounted() {
    ...
    if (this.$route.query && this.$route.query.page == 'Blog') {
      this.move(1)
    } else if (this.$route.query && this.$route.query.page == 'Comment') {
      this.move(3);
    } else {
      this.move(0);
    }
  ...
  methods: {
    ...
    async genCharts() {
      const data = {
        labels: [], datasets: [{
          name: "views", chartType: "line", values: []
        }]
      }
  ...
  <div :class="{ hidden: !login }" class="text-gray-500">
    ...
    <div class="w-full p-2 border-b mb-2 flex items-center ">
      ...
      <div class="flex overflow-x-scroll pb-2 sm:overflow-hidden">
        <div @click="idx != 2 ? move(idx) : $router.push('/plant')" :class="{ underline: nav[idx].active }"
    ...
    <div :class="{ hidden: !nav[0].active }" class="p-3">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 select-none">
        <!-- ultrasonik -->
        <div v-for="i, idx in hidroponik.ultrasonik"
          class="rounded-md p-2 m-2 w-36 sm:w-48 text-center bg-blue-500 text-white cursor-default">
        ...
        <!-- pompa w -->
        <div @click="pompaActivated(idx)" v-for="i, idx in hidroponik.pompa"
          class="rounded-md p-2 m-2 w-36 sm:w-48 text-center bg-lime-600 text-white sm:hover:opacity-60 cursor-pointer"
          :class="{ 'opacity-60': (hidroponik.mode) }">
          <span class="underline">pompa {{ namePompa(idx) }}</span><br>
          {{ (i) ? 'aktif' : 'non aktif' }}
        </div>
        <!-- mode w -->
        <div @click="(hidroponik.mode = !hidroponik.mode), refreshStatus = 'async'"
          class="rounded-md p-2 m-2 w-36 sm:w-48 text-center bg-lime-600 text-white sm:hover:opacity-60 cursor-pointer">
        ...
        <div class="rounded-md p-2 m-2 w-36 sm:w-48 text-center bg-lime-600 text-white cursor-pointer">
          ...
          <input @click="refreshStatus = 'async'" type="number" v-model="hidroponik.time_auto"
      ...
      <div class="flex text-center mt-5 border-t">
        <a href="https://www.niagahoster.co.id/"
          class="mt-2 border w-full mx-2 p-1 hover:underline bg-sky-500 text-white">domain</a>
      </div>
      <div class="flex text-center">
        <a href="https://app.supabase.com/"
          class="mt-2 border text-white w-full mx-2 p-1 hover:underline bg-gray-500">supabase</a>
        <a href="https://github.com/"
          class="mt-2 border text-white w-full mx-2 p-1 hover:underline bg-gray-500">github</a>
      </div>
    ...
    <div class="m-2" :class="{ hidden: !nav[1].active }">
      ...
      <form @submit.prevent="(blog.submit == 'update') ? submitContent('update') : submitContent('')"
          ...
          <tbody v-for="(i, idx) in blog.list.slice(blog.listIdx, blog.listIdx + blog.lengthlist + 1)">
            <tr class="text-sm text-center hover:bg-slate-200">
              ...
              <td class="p-1 bg-red-600 text-white sm:hover:underline cursor-pointer"
                @click="removeBlog(i.title, i.link)">delete</td>
              <td class="p-1 bg-amber-500 text-white sm:hover:underline cursor-pointer"
                @click="editBlog(i.blog_id, i.link)">edit</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="text-center">
        <button class="border px-2 mx-2 mb-2 hover:underline" :class="{ underline: (blog.listIdx == i - 1) }"
          @click="blog.listIdx = i - 1"
          v-for="i in Number((blog.list.length / blog.lengthlist).toFixed())">{{ i }}</button>
      </div>
        ...
        <div class="text-center text-gray-500 text-sm">
          ...
          <div class="text-white mt-4">
            ...
            <button class="ml-5 sm:hover:underline px-6 py-1 bg-sky-600"
              @click="blog.deleteModal = !blog.deleteModal">no</button>
          </div>
        </div>
      </Modal>
    ...
    <div v-if="nav[3].active" class="px-3 py-1">
        ...
        <table class="w-full">
          ...
          <tbody v-for="i, idx in comment.list.slice(comment.listIdx, comment.listIdx + comment.lengthlist + 1)">
            <tr class="text-sm hover:bg-slate-200 text-center">
              ...
              <td class="p-1 bg-red-600 text-white sm:hover:underline cursor-pointer"
                @click="removeComment(i.name + ': ' + i.comment, i.comment_id)">delete</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <!-- pagination -->
      <div class="text-center">
        <button class="border px-2 mx-2 mb-2 hover:underline" :class="{ underline: (comment.listIdx == i - 1) }"
          @click="comment.listIdx = i - 1"
          v-for="i in Number((comment.list.length / comment.lengthlist).toFixed())">{{ i }}</button>
      </div>
      ...
      <Modal class="fixed w-full h-full z-10 bg-opacity-90" :class="{ hidden: !comment.deleteModal }">
        <div class="text-center text-gray-500 text-sm">
          ...
          <div class="text-white mt-4">
            ...
            <button class="ml-5 sm:hover:underline px-6 py-1 bg-sky-600"
              @click="comment.deleteModal = !comment.deleteModal">no</button>
          </div>
        </div>
      </Modal>
